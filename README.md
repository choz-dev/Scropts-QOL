[![Releases](https://img.shields.io/github/downloads/Scroptss/Scropts-QOL/total.svg)](https://github.com/Scroptss/Scropts-QOL/) 


# Scropts QOL

## Features
- Menu Stutter / FPS Fix
- DLC Unlock
- Stat Editor 
- Force Host Zombie Matches (You can use this with a GSC menu)
- Auto Cryptokey + Divinium Farm + Spender 
- Spoof Unlock All: Weapons, Camos, Gobblegums, Black Market Items 
- Hard Unlock: Watermelon Camo, Dev / Beta Calling Cards, BM Contracts
- Name Spoofer & Clan Tag Editor
- Cbuf / Dvar Command Inputs 
- Download + Edit Emblems From Media Tab
- Edit MXT-Downloaded Emblems 
- Custom Profile Snapshot, Thumbnail, Title, and Description
- Full Create a Class Editor
- MP Character Customizer (White Hero Gear, Custom Showcase Weapons)
- Gobblegum Editor
- ZM Map High Round Editor
- ZM EE Completion Editor 
- Patches for Commonly Used Crashes and Kicks
 
## Download
The latest version can be found on the [Releases Page](https://github.com/Scroptss/Scropts-QOL/releases). 

## Credits
- [Serious](https://github.com/shiversoftdev): Custom Profile Snapshots

## Support Me
[![ko-fi](https://ko-fi.com/img/githubbutton_sm.svg)](https://ko-fi.com/Z8Z41Z1VT6)
